...
| 创建工序报工 | 新增不可变报工事实；异常数量还会新增待处置记录 | 已启用，scope `production.step-report.create.v2`；v2 返回最终目标、当前放行和可报数量；报工、异常待处置、工序状态、成功审计和幂等结果同事务 |
| 冲销报工 | 按原事实追加一条全量冲销；目标报工是唯一自然业务键 | 不发送 HTTP 幂等键；`UNIQUE(reversal_of_report_id)`、批次/工序状态和 `version` 提供天然幂等与并发保护，重放返回已存在的冲销事实 |
| 更正报工 | 在单个事务内追加全量冲销和替代事实；服务端单号不可复现 | 已启用，scope `production.step-report.correct.v2`；v2 返回最终目标、当前放行和可报数量；冲销、替代、异常待处置、工序状态、成功审计和幂等结果同事务 |
