...
逐项扩展：确认、分配、出库和库存流水命令须逐项满足启用门槛后再声明必填键，不得无差别给所有
POST/PATCH 加键。
...
| B. 命中判据二「需要」，按风险逐项启用（阶段 C）        | 成功响应丢失后的重试会因 `version` 已变而返回 409，需要结果重放                                              | `POST /api/production/work-orders/:workOrderId/actions/release`、`/cancel`、`/close`；批次状态确认（后续命令）；4.2 的物料分配、领料出库、库存流水                                                                                                 | 尚未启用；逐项风险评审并满足启用门槛后再声明必填键                                                                   |
