...
Confirmation-style commands must carry a unique `Idempotency-Key` HTTP header with 1 to 150 characters. The key is not duplicated in the body. A repeat with the same normalized request returns the original result; a reused key with a different request returns HTTP 409 with `IDEMPOTENCY_CONFLICT`. Product and Identity are not retrofitted in this phase, and no future business tables are created.
