...
短批授权必须绑定当时的物料计划版本。版本字段只保存在批次主表，`production_item_demand` 不增加版本字段，也不按“最新版本”筛选需求；每条需求仍由 `business_status = active/fulfilled/cancelled` 和 `remaining_number` 决定是否执行。批次维护 `material_plan_version`；创建或取消需求、开工前确认退料并释放回公共库存时在同一事务递增，授权保存 `partial_start_authorized_plan_version` 以及活动需求、确认出库和确认退料净额快照。开工事务要求两个版本相等且净领用量仍大于零。确认出库只会改善缺口，因此不递增该版本。该机制不拆分需求，也不允许修改既有 `need_number`；需求更正仍使用“取消旧需求并创建新需求”。
...
- 新工单只能冻结产品 `current_bom_version_id` 指向的当前已发布 BOM；草稿过期后必须显式刷新，下达时禁止继续使用历史 BOM。历史 BOM 只供历史工单查看和路线设计参考。
- 未来如需允许新工单选择历史 BOM，必须增加独立权限、强制原因、操作人/时间、全程警告和审计，并校验版本属于同产品、曾正式发布、不可变且存在兼容路线；不得直接移除下达时的当前 BOM 一致性校验。本期不实施。
