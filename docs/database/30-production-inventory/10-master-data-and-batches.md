...
本轮 BOM 版本整改采用严格当前版本规则：新建草稿工单只冻结 `products.current_bom_version_id`；下达时再次锁定产品并要求工单冻结版本仍等于当前版本，过期草稿必须由用户显式刷新后才能下达。历史 BOM 仅供历史工单查看和路线设计参考，普通工单创建、编辑和下达接口不得提供历史版本选择。

未来如需开放“按历史标准复产”，必须作为独立特批能力增加权限、选择原因、选择人/时间、全程警告和审计；后端只允许选择属于同产品且曾正式发布的不可变版本，并重新校验兼容路线。届时下达规则也必须显式区分当前版本与历史特批，不能通过删除当前版本一致性校验简单放开。本期不实施该例外。
