...
采购来源与事实边界已由 [ADR-0013](adr/0013-procurement-source-and-stock-boundaries.md) 明确，细化 ADR-0006 原来面向生产需求的采购门槛；采购能力仍未实施：

| 来源 | 选版和下单依据 |
| --- | --- |
| 按需求采购 | 关联具体 `production_item_demand.id`，继承正式需求的基础物料及精确版本，采购不能自行换版；BOM 已批准仍不足以下单，须先由任务形成需求，正式下单再通过所属模块公开能力复核需求资格、在审更正、任务状态和物料版本 |
| 备料采购 | 不依赖工单、生产任务或 BOM；采购自行选择可采购物料、有效精确版本、供应商和数量 |

两类来源共用一套采购单，实际供应商及正整数采购量均由采购确定；生产侧供应商期望只作提示，采购量不要求等于生产需求量。下单不自动预留，到货与入库不减少生产需求剩余量，后续仍通过分配和确认领料履约。来源需求更正、关闭、换版或任务结束不得自动改写、删除或改绑已订购事实；不新增第二份生产需求账本或需求专属库存隔离。

一期采购已确定不接在线审批，由采购正式确认下单，锁定供应商、物料版本和计划量；关键内容录错通过取消／结束后重建处理，不能因已有 Approval 就增加采购审批场景。外购强制检验、检验后由仓管确认入库；具体单据关系及仍待收敛的入库方式、到货纠错见[业务评审稿](procurement-inbound-design.md)，实施顺序见 [roadmap](roadmap.md)。受审快照只提供审批证据，不作为生产定义或可写业务影子表。
