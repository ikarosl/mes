...
[ADR-0011](adr/0011-task-closeout-output-list-and-finished-goods-inbound.md) 已确定任务结案须先由质检留存检验记录，产线管理员据此核对清单，再由所属工单负责人审批；质检无权改写产出处置单，不占审批节点。所需的业务关联人员分派扩展见 §7，尚未实施，不代表当前角色／指定用户节点已经支持该能力。
...
任务结案和批准清单更正的放行人必须是任务所属工单负责人。现有角色节点允许任一合格成员处理，指定用户节点将某个用户写入流程版本，均不能表达“不同工单使用各自负责人”。本节给出最小扩展方案；具体字段、契约与约束在实施时同步 Approval 所有者文档并追加 migration，当前表结构仍按 §6。
...
- 任务结案及清单更正的最终放行节点必须采用“工单负责人”来源；该约束在保存／发布和送审后端校验，不能靠界面默认值保证，不能配置为任意角色后绕过。至少一个工单负责人节点即可满足本需求，不强制两级；若配置其他管理节点，工单负责人仍负责最终放行。新规则实施时明确存量已发布配置和在途申请的处理，禁止静默重写历史或把旧审批补记为负责人批准。
- 送审缺少负责人、负责人账号无效或无审批权限时给出明确业务错误，不自动回退到申请人、管理员或同角色人员。当前工单负责人可空且普通编辑仅限草稿，实施时须将有效负责人前置为下达条件，送审再检查实际审批资格。
