...
- 前端数据必须先按 `frontend-architecture.md` 归类再实现：正式列表属于页面，跨页面同语义候选属于 Pinia
  共享参考 Store，单弹窗候选和 ID 绑定明细属于业务弹窗；禁止页面级大而全加载、模块级可变单例和重复原始缓存。
- 候选加载是 best-effort，单个资源失败不得拖累列表、其他候选或关键明细；共享候选已有成功快照时刷新失败
  保留最后成功值并标记陈旧，关键明细失败不得转换成可保存的空数据。下拉展开只刷新对应资源，写成功后只
  刷新受影响列表并失效准确候选。
