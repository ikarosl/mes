...
`production.step-report.correct.v3`），返工整单完成（scope
`production.rework.complete.v1`）和异常报废补料批准（scope
`production.abnormal.scrap-supplement-plan.confirm.v1`）；其余 Product、Identity、Production 端点误带任意
