...
- 前端依赖方向为“页面/业务弹窗 -> 用例 composable -> 跨页面 Pinia 参考 Store（如适用） -> `src/api`”；
  详细的数据分类、所有权和刷新矩阵见 `frontend-architecture.md`。
- 正式分页列表属于页面，ID 绑定关键明细属于当前业务弹窗；同一原始候选被多个页面消费时属于 Pinia，
  不得通过模块级单例或多个局部 `ref` 建立隐式缓存。
- 下拉展开只刷新自身资源；写操作成功只刷新受影响列表并失效准确的共享候选，禁止用全页面
  `loadData()` 或 `refreshAll()` 掩盖所有权不清。
- 异步请求必须具备与其作用域匹配的单飞、响应乱序和目标 ID 守卫；关键明细失败不得伪装成可保存的空数据。
