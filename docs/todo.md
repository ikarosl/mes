...
状态：`已完成（2026-08-24，整数口径已确认并落地）`

最终决策：

- 当前正式范围内所有参与业务运算或数量事实落库的值只允许整数；正数量最小为 `1`，允许空累计/正常异常拆分的字段可为 `0`，库存流水和盘点差异允许负整数。
- BOM 单位用量属于需求乘法输入，必须是整数。产品规格参数等只作为 JSON 原样记录且不参与业务计算的内容不套用整数数量规则。
- 不存在舍入口径：客户端、DTO、应用服务和数据库都必须拒绝小数，禁止四舍五入、截断或误差阈值比较。
- 单列继续受 `DECIMAL(12,4)` 既有范围限制，最大正整数为 `99999999`；乘法结果超出范围必须在写入前拒绝。

落地方式：

- 数据库保留精确 `DECIMAL(12,4)` 兼容表示，并由 `202608240001-integer-production-quantities` 为当前全部数量列追加整数 `CHECK`；历史小数会使 migration 失败并要求人工复核，不允许静默转换。
- HTTP DTO 使用整数与 `0..99999999` / `1..99999999` 值域校验；管理端数量控件统一 `precision = 0`、`step = 1`，正数量默认最小值为 `1`。
