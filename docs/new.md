...
| `expires_at`          | `DATETIME`                                  | 允许清理时间；第一阶段成功后至少保留 30 天         |
...
- 到期清理是运维回收，不改变 30 天内的协议保证。具体接口如需更长重试窗口，必须在接入时声明并延长；
...
历史 migration 完成状态：第 1～2 步由首批 migration 完成；2a 是后续登记的计划表，当前尚未迁移；
202607230001-product-master-data 合并实施了第 3～7 步（含 products.default_route_id 外键）。后续新表只能
追加新的 migration 文件。
