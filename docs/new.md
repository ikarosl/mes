...
约束：`UNIQUE (step_code, is_deleted)`；`default_sop_file_id -> technical_files.id ON DELETE SET NULL`。
...
约束：`product_id -> products.id`；`UNIQUE (product_id, route_code, version_no, is_deleted)`；启用后的路线版本不得原地修改步骤，只能创建新版本。
...
约束：`UNIQUE (route_id, step_order, is_deleted)`；`UNIQUE (id, route_id)`；检查布尔字段和 `step_order > 0`。
...
- 唯一约束：`UNIQUE (category_code, is_deleted)`
...
| `product_model`    | `VARCHAR(128)`    | 产品或物料型号，可为空                  |
...
- 主键：`id`
- 唯一约束：`UNIQUE (item_code, is_deleted)`
- 可选唯一约束：`UNIQUE (product_model, is_deleted)`，仅在型号不能为空且业务要求唯一时启用
...
- 唯一约束：`UNIQUE (product_id, material_product_id, is_deleted)`
...
- 唯一约束：`UNIQUE (reversal_of_transaction_id)`，同一原流水只能被完整冲销一次；部分冲销需求出现时需调整该规则
...
| `source_scrap_id`                  | `BIGINT UNSIGNED` | 报废补料关联的报废记录 ID                 |
...
- 检查约束：正常需求 `demand_type = 0` 时要求 `product_material_id IS NOT NULL`
...
| `status`                  | `VARCHAR(30)`     | `pending`、`assigned`、`doing`、`completed`、`abnormal`、`skipped` |
...
说明：创建生产批次时按路线步骤生成记录并复制快照；后续修改工序或路线不得回写已生成记录。
...
14. inspection_records / rework_records / finished_flow_records
15. 最后创建所有汇总视图
```
