...
- 表单下拉选择必须通过独立、最小字段的 `/options` 接口获取，禁止复用分页列表接口在浏览器过滤/切片；一个 `/options` 只承载一类可选项，不提供把多类选项打包返回的聚合端点（如 `form-options`）。前端只在弹窗打开等确需多类候选的生命周期并发组合独立 loader；展开某个下拉时只刷新对应资源，不得重新形成前端聚合端点语义。共享与局部候选边界见 `frontend-architecture.md`。
- `/options` 只返回表单需要的最小字段，并默认排除停用、删除记录；有明确小上限时不分页，规模增长后改为远程搜索。
