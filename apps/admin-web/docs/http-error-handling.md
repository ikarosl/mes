...
本文只描述当前仓库已经存在的前端基础设施，不扩展业务模块范围。
...
连接中断以及 `502/503/504` 才允许在复用原 `Idempotency-Key` 的前提下自动重试；
`IDEMPOTENCY_RESULT_CORRUPT` 即使是 500 也必须明确跳过重试。
...
| 7    | `409 IDEMPOTENCY_CONFLICT`                    | 统一提示提交标识已用于不同内容，要求刷新后重新操作                   |
| 8    | `503 IDEMPOTENCY_STORAGE_RETRYABLE`           | 统一提示提交服务暂时不可用，可稍后重试                               |
| 9    | `500 IDEMPOTENCY_RESULT_CORRUPT`              | 统一提示服务端保存结果异常，要求联系管理员并核对业务结果             |
| 10   | `500 INTERNAL_SERVER_ERROR` 或其他普通 500    | 保留通用服务器内部错误，不推断为幂等故障，并附带 `requestId`         |
| 11   | 其他错误                                      | 显示后端/标准化错误消息，缺失时使用通用失败提示                      |
...
HTTP 错误码描述失败来源；页面持有的幂等意图只描述“是否仍需保留原提交标识”。两者不能互相推断：

- 后端只有显式返回 `IDEMPOTENCY_CONFLICT`、`IDEMPOTENCY_STORAGE_RETRYABLE` 或
  `IDEMPOTENCY_RESULT_CORRUPT` 时，前端才可以把错误来源描述为幂等冲突或幂等存储故障；
- 指纹生成器、executor 或业务 handler 内部出现 `TypeError` 等未预期代码异常，统一属于
  `500 INTERNAL_SERVER_ERROR`。服务端日志记录异常类型和脱敏堆栈，前端不得因为接口恰好启用了幂等就追加
  “幂等结果异常”等诊断；
- 普通 500 或网络中断后，页面可以出于防重复目的在内存中保留原提交标识，但这只是恢复策略，不代表错误
  发生在幂等模块；
- 全局处理器负责 HTTP 错误的统一文案和 `requestId`，业务组件只负责 loading、弹窗、草稿、是否允许编辑及
  是否保留意图。业务组件不得再次解释相同 HTTP code，也不得用固定幂等文案覆盖全局错误；
- 尚未进入 HTTP 链路的本地错误（例如浏览器缺少必要能力）可由业务组件给出局部提示，但不能伪装成服务端
  响应。
...
- `apps/admin-web/src/api/__tests__/error-handler.test.ts`：验证 401 只处理一次、错误标准化、登录 401 保留后端
  消息、网络错误提示、三类幂等错误的全局映射，以及普通 500 只显示通用消息和 `requestId`。
- `apps/admin-web/src/utils/__tests__/message.test.ts`：验证已由全局处理的错误不会再次弹窗，本地错误仍会提示。
- `packages/request/src/__tests__/request.test.ts`：验证 RequestError 原样返回、AxiosError 标准化、幂等写请求
  不重试普通 500，并仅对临时 502/503/504 或无响应执行受控重试。

当前 `handleHttpError` 先判断 `status === 0`，再判断 `ECONNABORTED`；如果请求层把超时同时标准化为
`status === 0`，用户会看到网络错误而不是超时提示。调整顺序时必须补充超时优先级测试。
