 ...
 *  - 走 /production/work-orders/options 契约：后端完整返回 released/doing 且仍有余量的工单，前端本地 filterable。
...
export function useWorkOrderOptions() {
  ...
  const formatOption = (order: WorkOrderOption): string =>
    `${order.workOrderNo} / ${order.productCode} / ${order.productName} / 可分配 ${formatQuantity(
