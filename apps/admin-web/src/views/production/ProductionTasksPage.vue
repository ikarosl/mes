...
const loadOptions = async () => {
  try {
    const [formOptions, userOpts, sopFiles] = await Promise.all([
      ...
      productApi
        .technicalFiles({ page: 1, pageSize: 200, status: 1 })
...
const searchWorkOrders = async (keyword: string) => {
  ...
  try {
    ...
    const [released, doing] = await Promise.all([
    ...
    for (const order of [...released.items, ...doing.items]) {
...
const handleTaskOrderChange = async (workOrderId: string) => {
  ...
  const productRoutes = routeOptions.value.filter((r) => r.productId === order.productId);
  const defaultRoute = productOptions.value.find((p) => p.id === order.productId)?.itemCode
    ? productRoutes[0]
    : productRoutes[0];
  taskForm.routeId = defaultRoute?.id ?? '';
  taskForm.ownerId = '';
