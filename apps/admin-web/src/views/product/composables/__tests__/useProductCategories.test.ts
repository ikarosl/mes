...
const { categories, error } = vi.hoisted(() => ({ categories: vi.fn(), error: vi.fn() }));
vi.mock('../../../../api/product', () => ({
...
describe('useProductCategories', () => {
  beforeEach(() => {
    ...
    error.mockReset();
    categories.mockResolvedValue({ items: [], total: 0, page: 1, pageSize: 10 });
  });

  it('loads the category list with trimmed page query', async () => {
    const state = useProductCategories();
    state.query.categoryCode = '  MAT ';
    state.query.categoryName = '电子';
  ...
  it('discards a late response after the query changes', async () => {
    ...
    categories.mockImplementation((params: { page: number }) =>
      params.page === 1
        ? new Promise((resolve) => {
    ...
    const pendingPage1 = state.loadCategories();
    const pendingPage2 = state.handlePageChange(2);
    await pendingPage2;
    resolvePage1(pageResult([{ id: '1' }], 5, 1));
    await pendingPage1;

    expect(state.categories.value).toEqual([{ id: '2' }]);
