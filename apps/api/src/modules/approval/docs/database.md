...
编辑节点须先锁定义、再锁此版本并核对草稿 ID 和乐观锁，保存完整顺序后递增 `version`；发布按定义、版本的固定顺序锁定，检查至少一级、连续顺序、角色或指定用户有效及当前合格人员；业务来源节点只检查场景支持及末级来源约束，不预解析具体工单，再原子写发布状态与定义指针。新版本号在定义锁内分配。`discarded` 为保留状态，本次未开放废弃草稿接口，不物理删除或覆盖已发布版本。
...
业务人员迁移 `202609160001` 增加来源代码、三选一 CHECK、实例解析用户 FK 及检索索引。升级暂停 Approval 和相关业务写入，不为旧版审批增加兼容分支；开发数据可重置后重新发布符合末级负责人规则的流程。down 在任何持久 DDL 前拒绝已有业务来源配置（含软删除历史）或解析用户记录，不能降为固定用户或删除审批证据。开发环境可统一重置，不新增双写／影子表。
