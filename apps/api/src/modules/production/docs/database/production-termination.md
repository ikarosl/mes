...
- 正常执行完成仍校验全部工序、需求、补料和末工序数量；记录 `execution_completed_at/by`，由 `doing` 进入 `closing`，建立 `normal` 结案草稿，不在批次表存储报工汇总数量。此时尚未最终结案。
...
提前结束清单首次批准为 `terminated` 后，原任务计划不再占工单分配额度；`closing` 期间仍占用。原计划、领料及批准产出保留，额度规则与工单选版解除条件见[工单设计](work-orders-and-batches.md)。
...
- 完全没有产出仍须质检人员核实并留存三项均为零的记录、真实结论及凭据，不豁免质检或批准清单关联。全部报废须据实填写新增成品报废，历史工序报废不重复计入；两类可入库数量均为零时不办理成品入库。
...
`result_note/evidence_reference` 必填且不能仅含空白，接口分别限制为 5000 字符；`evidence_reference` 记录真实的线下质检单编号或凭据存放位置，零产出确认适用同一要求。数据库保留非空及非空白 CHECK，不因没有可入库成品放宽。
...
当前审批证据结构仅接受版本 `4`，包含结案模式、前版、更正原因、具体质检事实、产出三项、原收尾处理与逐笔物料损耗证据，以及 `workOrderOwnerEvidence`。不读旧版、不补造缺失快照；开发数据允许重置。
