...
export class ProductionTraceQueries {
  ...
  async search(query: ProductionTraceQuery): Promise<PageResult<ProductionTraceWorkOrderGroup>> {
    ...
    if (query.keyword?.trim()) {
      ...
      const batchKeywordQuery = `SELECT 1 FROM production_item_allocation a JOIN item_batch ib ON ib.id=a.batch_id
  ...
  async listMaterialInboundSources(
    batchId: string,
    ...
    return rows.map((row) => ({
      ...
      inboundQuantity: String(row.quantity),
...
const mapSummary = (row: TraceSummaryRow): ProductionTraceBatchSummary => ({
  ...
  executionCompletedAt: row.execution_completed_at
    ? toBeijingISOString(row.execution_completed_at)
    : null,
  finalOutput:
    row.current_revision_id === null
      ...
      : {
          ...
          scrapQuantity: String(row.scrap_quantity),
          plannedShortfallQuantity: String(
            Number(row.planned_quantity) - Number(row.available_quantity),
          ),
        },
