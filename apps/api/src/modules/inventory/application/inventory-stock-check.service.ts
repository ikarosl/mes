...
import { InventoryStockCheckRepository } from './ports/inventory-stock-check.repository.js';

@Injectable()
export class InventoryStockCheckService {
  constructor(
    private readonly stockChecks: InventoryStockCheckRepository,
...
const requireReason = (value: string): string => {
  ...
  if (!reason) throw new InventoryDomainError('INVALID_INPUT', '取消原因不能为空');
  return reason;
};
const requireUnique = (values: string[], message: string): void => {
  if (new Set(values).size !== values.length)
    throw new InventoryDomainError('INVALID_INPUT', message);
};
