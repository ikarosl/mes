...
演示成品的 BOM 默认 `draft`，不生成审批配置、申请或批准记录。生产管理员演示角色具备审批查看和处理权限；管理员可在审批流程配置页选择“生产管理员 → 系统管理员”两级，使用 `production-001` 和 `.env` 初始化的管理员分别处理。配置与重新分派权限仍由系统管理员管理，演示操作工不获得审批资格。
